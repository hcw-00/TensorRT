...
class TransformerDecoderEngineBuilder(DefaultEngineBuilder):
    """
    Parses an ONNX graph and builds a TensorRT engine from it.
    """

    def __init__(self, verbose=False, workspace=8, calib_batch_size=1, max_len=26):
        """
        :param verbose: If enabled, a higher verbosity level will be set on the TensorRT logger.
        :param workspace: Max memory workspace to allow, in Gb.
        """
        ...
        tgt_op_shape = [(1, calib_batch_size, 512), (max_len//2, calib_batch_size, 512), (max_len, calib_batch_size, 512)]
        profile.set_shape("tgt", *tgt_op_shape) #TODO fix hardcoding
    ...
    def create_network(self, onnx_path):
        ...
        log.info("Network Description")
        for input in inputs:
            self.batch_size = input.shape[1] # batch_size is second parameter
            log.info("Input '{}' with shape {} and dtype {}".format(input.name, input.shape, input.dtype))
        for output in outputs:
            log.info("Output '{}' with shape {} and dtype {}".format(output.name, output.shape, output.dtype))
        # assert self.batch_size > 0
...
def main(args):
    if isinstance(args.model_type, str) and args.model_type.lower() == "decoder":
        builder = TransformerDecoderEngineBuilder(args.verbose, args.workspace, args.calib_batch_size)
    else:
        builder = DefaultEngineBuilder(args.verbose, args.workspace)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", default=None, type=str, help="The specific model type to build.")
