...
__launch_bounds__(nthdsPerCTA) __global__
    void testKernel(const int n, const float coefficient, const float* input, float* output)
{
...
pluginStatus_t testGPU(cudaStream_t stream, const int n, const float coefficient, const void* input, void* output)
{
    const int BS = 512;
    const int GS = (n + BS - 1) / BS;
    testKernel<BS><<<GS, BS, 0, stream>>>(n, coefficient,
                                           (const float*) input,
                                           (float*) output);
...
{
    return testGPU(stream, n, coefficient, (const float*) input, (float*) output);
}